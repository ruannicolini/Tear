...
{     Many thanks to Eugene Genev for his XiComponents                   }
{
      ...
      21/01/2005 Punlished on http://dade2000.altervista.org
{************************************************************************}
...
type
  ...
  TProgressColorScheme = (procsCustom, procsDesert, procsGrass, procsSilver,
    procsSky, procsRose, procsSun,
    procsHackers, procsNight, procsVelvet, procsMetal, procsViolet, procsToxic);
...
function MessageDlgPosI(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer; Lang: TLangType):
  Integer;
...
function InputBoxI(const ACaption, APrompt, ADefault: string; Lang: TLangType):
  string;
...
begin
  ...
  begin
    ...
    raise
      EMathError.Create('Please specify at least two colors');
end;
    ...
    for B := Low(TMsgDlgBtn) to High(TMsgDlgBtn) do
      ...
      begin
        ...
        begin
          ...
          begin
            ColorScheme :=
              XiButton.TColorScheme(MsgOptions.CustomButtonsColorScheme);
            Name := ButtonNames[B];
            Parent := TheParent;
...
begin
  ...
  begin
    ...
    Parent := Panel;
    ColorScheme :=
      XiProgressBar.TColorScheme(MsgOptions.CustomProgressColorScheme);
  end;
  ...
  else if (MsgOptions.ShowStatusOnlyOnForm = 0) or (GetForegroundWindow =
    MsgOptions.ShowStatusOnlyOnForm) then
  begin
